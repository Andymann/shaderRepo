/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#65901.0"
}
*/
...
const float rad7 = .5;
 ...
 float yDiff = (2.*apothem7*sin(.5*PI/7.));
 float h = sqrt(apothem7*apothem7*4. - (width7 + width7s)*(width7 + width7s));

 vec2 s = vec2(width7*2. + width7s*2., (apothem7 + apothem7 + h));
 vec2 s2 = s*vec2(1, 2);
 float yh = s.y - apothem7 - rad7;
