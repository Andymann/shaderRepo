/*
{
  ...
  "DESCRIPTION" : "Automatically converted from https:\/\/www.shadertoy.com\/view\/4dfGzs by iq.  Correct edge detection for voxels. The marching function is fb39ca4's DDA, but using floating point operations instead of integers. The most interesting bits are probably the exact 3D intersector, the occlusion and the edge detection code.",
  "INPUTS" : [
    {
      "NAME" : "iMouse",
      "TYPE" : "point2D"
...
float noise( in vec3 x )
{
	...
	vec2 rg = textureLoad( iChannel0, (uv+0.5)/256.0, 0.0 ).yx;
...
vec3 render( in vec3 ro, in vec3 rd )
{
	...
	if( t>0.0 )
	{
        ...
        col = 2.0*IMG_NORM_PIXEL(iChannel1,mod(0.01*pos.xz,1.0)).zyx; 
