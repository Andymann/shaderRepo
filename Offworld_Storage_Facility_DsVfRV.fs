/*
{
    ...
    "DESCRIPTION": "Automatically converted from https://www.shadertoy.com/view/DsVfRV by Shane.  Using a mixture of raymarching and cell by cell traversal techniques to raymarch a pseudo random box subdivision in order to render a basic sci-fi looking scene in realtime.",
    "IMPORTED": {
        "iChannel1": {
            "NAME": "iChannel1",
...
float hm(in vec2 p){ 
    ...
    //float h = getTex(BufferA, p/32.).x; 
    float h = dot(getTex(BufferA, p/32.), vec3(.299, .587, .114)); 
...
vec3 normal(in vec3 p) {
    ...
    for(int i = min(FRAMEINDEX, 0); i<6; i++){
...
vec3 render(vec3 ro, vec3 rd, vec3 light, inout float tt){
  ...
  if (t < FAR) {
    ...
    vec3 refTx = textureCube(iChannel3,ref).xyz; refTx *= refTx; // Cube map.
	...
	if (PASSINDEX == 0)	{
	  ...
	  vec2 uv = (gl_FragCoord.xy - RENDERSIZE.xy*.5)/RENDERSIZE.y;
	   ...
	   // Light position offset. Redundant here.
	    ...
	    vec4 preCol = texelFetch(BufferA, ivec2(gl_FragCoord.xy), 0);
	    float blend = (FRAMEINDEX < 2) ? 1. : 1./2.; 
	    gl_FragColor = mix(preCol, vec4(clamp(col, 0., 1.), t), blend);
	    #else
