/*
{
    ...
    "DESCRIPTION": "Automatically converted from http://glslsandbox.com/e#67116.0",
    "INPUTS": [
    ]
}
...
void _main (out vec4 _gl_FragColor, in vec2 _gl_FragCoord) {
	...
	float rayStereo = 0.5 * sign (_gl_FragCoord.x - RENDERSIZE.x * 0.5) * step (0.5, VR);
	vec3 rayOrigin = headPosition + headRotate * (headModel + vec3 (rayStereo * IPD, 0.0, 0.0));
	vec3 rayDirection = headRotate * normalize (vec3 ((2.0 * _gl_FragCoord.x - (1.0 + rayStereo) * RENDERSIZE.x), 2.0 * _gl_FragCoord.y - RENDERSIZE.y, 0.5 * RENDERSIZE.x / tan (FOV * PI / 360.0)));
