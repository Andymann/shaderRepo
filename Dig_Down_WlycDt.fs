/*
{
    ...
    "DESCRIPTION": "Automatically converted from https://www.shadertoy.com/view/WlycDt by Kali.  Yet another trippy tunnel still",
    "IMPORTED": {
    },
    "INPUTS": [
    ]
...
vec3 fractal(vec3 p)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        p.xy *= rot(z);
        p = abs(p * 1.5) - 2.;
        m = min(m, abs(p.y) + .5 * abs(.5 - fract(p.x * .25 + TIME + float(i) * .1)));
    }
    m = exp(-4. * m) * 2.;
