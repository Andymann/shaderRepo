/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#47960.0"
}
*/
...
void main( void ) {
	...
	vec3 cp = vec3(cos(TIME * 0.2), 0.0, sin(TIME * 0.45)) * .5;
	vec3 cl = vec3(-sin(TIME), 3.0, cos(TIME));
	vec3 cf = normalize(cl - cp);
	vec3 cs = normalize(cross(cf, vec3(sin(TIME * 0.1), -1.0, cos(TIME * 0.1))));
	...
	float a = (atan(rp.z, rp.x)) * 3.0 / PI;
	float ai = floor(a);
	float af = fract(a);
	float d = (rp.y + 0.5 * TIME) * 2.0;
	float di = floor(d);
	float df = fract(d);
