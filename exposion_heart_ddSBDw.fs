/*
{
    ...
    "DESCRIPTION": "Automatically converted from https://www.shadertoy.com/view/ddSBDw by zyc.  exposion heart",
    "IMPORTED": {
    },
    "INPUTS": [
    ]
    ...
    for (float i = 0.; i < np; i++)
    {
        ...
        if (v.y > -6.5 && v.y < 0.0 && t_i >= t_up /*&& SPAWN == 1*/)
        {
            ...
            for (float j = 0.0; j < snp; j++)
            {
                ...
                /// --------------------------------------------------
                vec3 pcol = 0.5 + 0.5*cos(TIME+uv.xyx+vec3(0,2,4));
