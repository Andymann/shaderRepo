/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#66562.3"
}
*/
...
const float rotation_speed = 0.14;
const float t2 = 2.0; // Length in seconds of the effect
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    //p += vec2(0.0, -0.1);
	float xx = dot(cos(p),p);
	p.y = sin(TIME+dot(sin(p),p)*2.0);
	p.x = sin(TIME*0.25+xx*2.0);
    ...
    float val = 0.45 + 0.55 * fBm3(
        vec3(vec2(1.0, 0.2) * polar, 0.15 * iTime));
    val = clamp(val, 0.0, 1.0);
    col.rgb = vec3(sin(TIME*0.8)+0.55, sin(TIME*0.4)+0.6, 0.7) * vec3(val);
    
    // Add white spots
    vec3 white = 0.65 * vec3(smoothstep(0.55, 1.0, val));
    col.rgb += white;
