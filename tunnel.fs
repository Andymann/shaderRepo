/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#37694.2",
  "INPUTS" : [
    {
      "NAME" : "DepthDistance",
      "TYPE" : "float",
...
float distfunc(vec3 pos)
{
	...
	return min(max(max(
	periodic(r,3.0,0.2),
	periodic(pos.z,1.0,0.7+0.3*cos(DepthDistance))),
	periodic(a*r,3.141592*2.0/6.0*r,0.7+0.3*cos(BrickDistance))),
	0.25);
...
void main()
{
	vec2 coords=(2.0*gl_FragCoord.xy-RENDERSIZE)/max(RENDERSIZE.x,RENDERSIZE.y);
