/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#52046.2"
}
*/
...
void main( void )
{
    vec2 uv = (gl_FragCoord.xy-RENDERSIZE.xy*vec2(.5,0))/RENDERSIZE.y;
    uv = uv*2.-vec2(0,0);
    ...
    gl_FragColor = vec4(.5+.5*sdf/(abs(sdf)+.002)-r,// remove r for original
			.5+.5*sdf/(abs(sdf)+.002)-r,
			.5+.5*sdf/(abs(sdf)+.002)+r,1.0); // map SDF to slightly soft black & white image
}