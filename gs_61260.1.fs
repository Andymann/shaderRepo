/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#61260.1"
}
*/
...
//---- This is a test to get it working in ISF Editor
//----
...
void initGlobal(vec2 fragCoord, vec3 RENDERSIZE, vec4 mouse, float TIME)
{
    text.size = ivec2(16,32)*2;
    text.adv  = ivec2(text.size.x,0);
    
    mouse.xy = min(mouse.xy,RENDERSIZE.xy);
    if (mouse.z < 1.0)
    {
        if (mouse.z > -1.0)
            gl.mouse = RENDERSIZE.xy*0.5;
        else
            gl.mouse = mouse.xy;
    }
    else gl.mouse = mouse.xy;
    
    gl.mp = (2.0*abs(gl.mouse)-vec2(RENDERSIZE.xy))/RENDERSIZE.y;    

    gl.aspect = RENDERSIZE.x / RENDERSIZE.y;
    gl.frag   = fragCoord;
    gl.ifrag  = ivec2(fragCoord);
    gl.uv     = (fragCoord+fragCoord-RENDERSIZE.xy)/RENDERSIZE.y;
