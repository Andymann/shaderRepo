/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#47833.1"
}
*/
...
{
    float d=0.;
    d = square(vec2(.0, 0.), rotate(-3.5, uv-vec2(.035,-.1)), vec2(.02, 0.002)) ; // mouth1
    d = add( d, square(vec2(.0, 0.), rotate(3.5, uv-vec2(.0,-.1)), vec2(.02, 0.002)) ); // mouth2
    d = add( d, ellips( vec2(-.1, .0), rotate(.1,uv), vec2(1.5,1.4), .006) ); // eye1
    d = add( d, ellips( vec2(.1, -.01), rotate(-.1,uv), vec2(1.5,1.4), .006) ); // eye2
...
{
    float d=0.;
    float x=sin(TIME*2.)*.06;
    float y=sin(TIME+hash(PI))*.004;
...
void main()
{
    ...
    vec2 uv = 1.5*gl_FragCoord.xy/RENDERSIZE.xy;
     vec2 uv2 = 1.5*gl_FragCoord.xy/RENDERSIZE.xy;
	...
	uv.y =  uv.y+.12-abs(sin(TIME));
	
    
    // shapes
    vec2 ruv=rotate(sin(TIME)*.20,uv);
    float hd = head( ruv );
	float fd = face( ruv );
    float bd = body( uv );
    ...
    // materials
    //vec3 bgb   = vec3(.0, .0, .0) * pow( (1.3-length(uv)), .2 );
	vec3 bgb   = vec3(.0, .0, .0);
    vec3 black = vec3(.3);
    vec3 white = vec3(1.);
	...
	col += strokestep( AA, M, ld, vec3(0.4+sin(TIME),1.0,0.0), black, bgb );
    

    // Output to screen
    gl_FragColor = vec4(col,1.0);
	
	if (uv2.y<0.05) gl_FragColor = vec4(1.0,0.0,1.0,1.0);
	 
		
		
}
// --------[ Original ShaderToy ends here ]---------- // thx gigatron
