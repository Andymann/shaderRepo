/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#56763.0"
}
*/


// @fizzer partycoding at BRBTITAN
// played abit with your logo... maybe add a sinus to pos and the letters will move ;-)
//or more letters and you got a nice scrolltext / greetings text...-Harley
// TITAAAAAN!!!!!
...
float scene(vec3 p)
{
   ...
   p = rotateX(cos(1.0 * 1.0 + 1.0 * 1.0) * 0.3, p);
...
/*
   ...
   d = min(d, cube(p, vec3(0.3, -0.08, 0.2), vec3(0.05, 0.12, 0.1))); // I
   d = min(d, cube(p, vec3(0.3, 0.15, 0.2), vec3(0.05, 0.05, 0.1))); // I
	
   d = min(d, cube(p, vec3(0.6, 0.0, 0.0), vec3(0.1, 0.2, 0.1))); // T
   d = min(d, cube(p, vec3(0.6, 0.2, 0.0), vec3(0.2, 0.05, 0.1))); // T
	
   d = min(d, cube(p, vec3(1.05, 0.15, 0.2), vec3(0.2, 0.05, 0.1))); // A
   d = min(d, cube(p, vec3(0.9, 0.0, 0.2), vec3(0.05, 0.2, 0.1))); // A
   d = min(d, cube(p, vec3(1.2, 0.0, 0.2), vec3(0.05, 0.2, 0.1))); // A
   d = min(d, cube(p, vec3(1.05, 0.0, 0.2), vec3(0.2, 0.02, 0.1))); // A
...
vec3 stars(vec2 p)
{
   ...
   float b = 0.;// = fract(atan(p2.y, p2.x) / 3.1415926 * 10.0 + length(p2) * 3.0 * cos(TIME * 0.3) + cos(TIME * 0.5) * 2.0);
...
void main()
{
   vec2 t = gl_FragCoord.xy / RENDERSIZE.xy * 2.0 - vec2(1.0);
