/*
{
  ...
  "DESCRIPTION" : "Automatically converted from https:\/\/www.shadertoy.com\/view\/MlXGWf by Xor.  Look around with the mouse. I've seen some nice water scenes, so here's my attempt.\nLet me know what you think in the comments. Suggestions are appreciated.\nUpdate 3\/6\/15:\nSmoothed water and fixed the stretched rocks.",
  "INPUTS" : [
    {
      "NAME" : "iMouse",
      "TYPE" : "point2D"
...
{
    vec2 dir = ((iMouse.xy/RENDERSIZE.xy)*vec2(1.0,-0.5)+vec2(0.0,0.75))*6.28;
...
vec3 doBackground( in vec3 dir)
{
    ...
    vec3 ground = IMG_NORM_PIXEL(iChannel0,mod((dir.xz)/dir.y,1.0)).rrr*vec3(1.1,1.0,0.9);
