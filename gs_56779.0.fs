/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#56779.0"
}
*/


//----Based on http://glslsandbox.com/e#56711.0
//----I need some of this for later
...
const float recursion_level = 1.0;
...
void main(void)
{
	...
	float scale = 1.;
	vec2 position = vec2((gl_FragCoord.x / RENDERSIZE.x) - 0.5, (gl_FragCoord.y / RENDERSIZE.y) - 0.5) * scale/* + mouse * 16.0*/;
	position =vec2((gl_FragCoord.x / RENDERSIZE.x) - 0., (gl_FragCoord.y / RENDERSIZE.y));
...
//	position = rotated_position;
	...
	vec2 effect_group = floor(position) * 6.0; // effect group float id
	...
	for (float ii=0.0; ii<=recursion_level; ii++)
	{
		...
		else if (effect_number==5.0) // tunnel
		{
			u = 8.0 / dist_from_center + 16.0 * TIME/2.;
			v = angle_from_center * 16.0;
	 
			fade = dist_from_center * 1.0;
			gradient = (sin(u) + sin(v)) * fade;
		...
		else if (effect_number==7.0) // blobs
		{
			gradient = 1.0;
			...
			gradient = sin(gradient) * sin(gradient) * 16.0;
			color = vec3(gradient / 1.5,  gradient / 2.0, gradient * 1.5);
			break;
		}
		else if (effect_number==8.0) //bumpmap
		{
			float disp = 0.005;
		...
		else if (effect_number==11.0)
		{
			...
			vec2 tex = (centered_coord * 16.0) / z;
