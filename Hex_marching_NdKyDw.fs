/*
{
    ...
    "DESCRIPTION": "Automatically converted from https://www.shadertoy.com/view/NdKyDw by mrange.  License CC0: Hex Marching\nResults from saturday afternoon tinkering\n",
    "IMPORTED": {
    },
    "INPUTS": [
    ],
...
vec3 dblur(vec2 q,float rad) {
  ...
  for (int j=0; j<iter; ++j) {  
    rad += 1./rad;
    angle*=brot;
    vec4 col=IMG_NORM_PIXEL(BufferB,mod(q+pixel*(rad-1.)*angle,1.0));
    acc+=col.xyz;
  }
...
void main() {
	if (PASSINDEX == 0)	{
	  ...
	  vec2 q = gl_FragCoord.xy/RESOLUTION.xy;
	  vec2 p = -1.0+2.0*q;
	  vec4 pcol = IMG_NORM_PIXEL(BufferA,mod(q,1.0));
	  ...
	  vec4 pcol = IMG_NORM_PIXEL(BufferB,mod(q,1.0));
	  vec3 col = pcol.xyz;
