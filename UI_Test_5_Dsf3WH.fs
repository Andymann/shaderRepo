/*
{
    ...
    "DESCRIPTION": "Automatically converted from https://www.shadertoy.com/view/Dsf3WH by yasuo.  3D ring UI component",
    "IMPORTED": {
    },
    "INPUTS": [
        {
...
float ring0(vec2 p){
    ...
    p = prevP;
    p*=Rot(radians(-TIME*50.+30.));
    p+=sin(p*25.-radians(TIME*80.))*0.01;
    d2 = abs(length(p)-0.65)-0.0001;
    d = min(d,d2);
    
    p = prevP;
    a = radians(-sin(TIME*1.2))*120.0;
    a+=radians(-70.);
    p.x+=cos(a)*0.58;
    p.y+=sin(a)*0.58;
...
float numberWithCIrcleUI(vec2 p){
    ...
    int num = int(mod(TIME*6.0,10.0));
    float d = drawFont(p-vec2(-0.16,0.),num);
    num = int(mod(TIME*3.0,10.0));
    ...
    p*=1.5;
    num = int(mod(TIME*10.0,10.0));
    d2 = drawFont(p-vec2(0.04,-0.03),num);
    d = min(d,d2); 
    num = int(mod(TIME*15.0,10.0));
...
float rectUI(vec2 p){
    ...
    p*=Rot(radians(-45.));
    p.y = abs(p.y)-0.07-sin(TIME*3.)*0.01;
...
float graphUI(vec2 p){
    ...
    float w = (abs(sin(TIME*n)+0.25)*0.03)*n*0.5;
...
void main() {
    ...
    vec3 ro = vec3(0, 0, -2.1);
    {
        ...
        ro.xz *= Rot(radians(sin(TIME*0.3)*animRXZ+ogRXZ));
    }
